...
int catalan_number_calculatio(int n) {
	if ( n == 0 or n == 1) return 1;
	int ans = 0;
	for(int i = 1; i < n + 1; i++) {
		int x, y;
		
		 x = catalan_number_calculatio(i - 1);
		
		  y = catalan_number_calculatio(n - i);
...
int count_of_bst(int n, int dp[]) {
	...
	int ans = 0;
	for(int i = 1; i < n + 1; i++) {
		int x = count_of_bst(i - 1, dp);
		int y = count_of_bst(n - i, dp);
		ans += x * y;
	}
	dp[n] = ans;
	return ans;
...
void solve() {
	...
	dp[0] = dp[1] = 1;
	for(int i = 0; i < n + 1; i++) { 
		cout << i << "= " << count_bst_bottom_up(i) << endl;
	}
