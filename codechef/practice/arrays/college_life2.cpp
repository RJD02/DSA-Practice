...
void solve() {
	int n;
	cin >> n;
	vector<vector<int>> season_timing;
	vector<int> intros(n);
	ll sum = 0;
	for(int i = 0; i < n; i++) 
		cin >> intros[i];
	for(int i = 0; i < n; i++) {
		int ne;
		cin >> ne;
		vector<int> timing(ne);
		for(int j = 0; j < ne; j++)
			cin >> timing[i];
		season_timing.pb(timing);
	}
	// for(auto a: season_timing) {
	// 	for(auto i: a) 
	// 		cout << i << " ";
	// 	cout << endl;
	// }
	for(int i = 0; i < season_timing.size(); i++) {
		auto episodes = season_timing[i];
		for(int j = 0; j < episodes.size(); j++) {
			if(j == 0)
				sum += episodes[j];
			else
				sum += episodes[j] - intros[i];
		}
	}
	cout << sum << endl;
}
