...
void solve() {
	...
	auto bfs = [&] (int i, int j) {
		...
		while(!q.empty()) {
			...
			for(auto [dx, dy] : d4) {
				int xx = x + dx, yy = y + dy;
				if(isInside(xx, yy) and world_map[xx][yy] == '1') {
					world_map[xx][yy] = '0';
					q.push({xx, yy});
				}
			}
	...
	for(int i = 0; i < n; i++) {
		for(int j = 0; j < m; j++) {
			if(world_map[i][j] == '1')
			component_sizes.pb(bfs(i, j));
		}
	}

	sort(all(component_sizes), greater<int>());

	int chef = 0;
	for(int i = 1; i < sz(component_sizes); i += 2) {
		chef += component_sizes[i];
	}
	cout << chef << endl;
}
