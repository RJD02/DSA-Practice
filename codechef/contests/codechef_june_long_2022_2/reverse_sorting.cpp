...
void merge(vi v, int begin, int mid, int end) {
	int sub_array_one = mid - begin + 1;
	int sub_array_two = end - mid;

	vi left_array(sub_array_one), right_array(sub_array_two);

	for(int i = 0; i < sub_array_one; i++) {
		left_array[i] = v[begin + i];
	}
	...
	while(index_sub_one < sub_array_one and index_sub_two < sub_array_two) {
		if(left_array[index_sub_one] <= right_array[index_sub_two]) {
			v[index_merged_arr] = left_array[index_sub_one];
			index_sub_one++;
		} else {
			v[index_merged_arr] = right_array[index_sub_two];
			index_sub_two++;
		}
		index_merged_arr++;
	}

	while(index_sub_one < sub_array_one) {
		v[index_merged_arr] = left_array[index_sub_one];
		index_sub_one++;
		index_merged_arr++;
	}
	while(index_sub_two < sub_array_two) {
		v[index_merged_arr] = right_array[index_sub_two];
		index_sub_two++;
		index_merged_arr++;
	}
...
int sum(vi v, int begin, int end) {
	int count = 0;
...
bool isAlreadySorted(vi v, int begin, int end) {
...
int mergeSort(vi v, int begin, int end, int x) {
	if(begin >= end or isAlreadySorted(v, begin, end)) {
		return 1;
	}

	int mid = begin + (end - begin) / 2;

	int sum_of_left_array = sum(v, begin, mid);
	int sum_of_right_array = sum(v, mid + 1, end);

	
	if(mid - begin < 2 and (sum_of_left_array > x or sum_of_right_array > x)) {
		return 0;
	}
	int n, m;
	n = m = 1;
	if(sum_of_left_array > x)
		n = mergeSort(v, begin, mid, x);
	if(sum_of_right_array > x)
		m = mergeSort(v, mid + 1, end, x);
	return min(n, m);
}
