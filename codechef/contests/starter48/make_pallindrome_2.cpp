...
string makePalindrome(string str, int k) {
	if(isPalindrome(str) or !k)
		return str;
	// bug(str);
	string left = makePalindrome(str.substr(1, str.size() - 1), --k);
	if(!left.empty())
		return left;
	string right = makePalindrome(str.substr(0, str.size() - 1), --k);
	if(!right.empty())
		return right;
}
...
void solve() {
	...
	string s;
	cin >> s;
	string ans = makePalindrome(s, floor(n / 2));
	cout << ans << endl;
}
