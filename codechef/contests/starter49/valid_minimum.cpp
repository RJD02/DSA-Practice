...
// 	int maxValue = max(minAB, max(minBC, minAC));
// 	bool okay = 0;
// 	for(int i = 0; i <= maxValue; i++)
// 		for(int j = 0; j <= maxValue; j++)
// 			for(int k = 0; k <= maxValue; k++)
// 				if(min(i, j) == minAB and min(j, k) == minBC and min(i, k) == minAC)
// 					okay = 1;
// 	okay ? cout << "YES" : cout << "NO";
// 	cout << endl;
// }
...
while(t--)
{
    int n1,n2,n3;
    cin>>n1>>n2>>n3;
    //int res = max(min(n1,n2),min(n2,n3),min(n3,n1));
    //cout<<res;
    if(n1==n2&&n2==n3)
    {
        cout<<"yes"<<endl;
