...
ll noString(int i, string &s, vi &dp, map<string, string> &code, string curr, set<string> &st) {
	if(i < 0) {
	...
	if(dp[i] != -1)
		return dp[i];
	ll one = noString(i - 1, s, dp, code, code[s.substr(i, 1) + curr], st);
	ll two = 0;
	if(i - 1 >= 0 and code.find(s.substr(i - 1, 2)) != code.end())
		two = noString(i - 2, s, dp, code, code[s.substr(i - 1, 2)] + curr, st);
	return dp[i] = (one + two) % 998244353;
}
...
void solve() {
	...
	code["ba"] = "101";
	set<string> st;
	cout << noString(n - 1, s, dp, code, "", st) << endl;
}
