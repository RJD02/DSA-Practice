...
vpi two_sum(vi nums, int target) {
	int start, end;
	vpi v;
	start = 0;
	end = sz(nums) - 1;
...
// Solving triplets using two sum algorithm
void solve() {
	int n, target;
	cin >> n >> target;
	vi v(n);
	...
	for(int i = 0; i < n - 2; i++) {
		vi to_pass(v.begin() + i, v.end());
		for(auto x: two_sum(to_pass, target - v[i])) {
			res.pb({v[i], x.f, x.s});
		}
	}
	for(auto x: res) {
