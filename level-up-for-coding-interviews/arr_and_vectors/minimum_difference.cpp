...
int binsearch(vi b, int val) {
	int low = 0;
	int high = b.size() - 1;
	while (low < high) {
		int mid = (low + high) / 2;
		if(abs(b[mid] - val) > abs(b[mid - 1] - val)) {
			high = mid - 1;
		} else if (abs(b[mid] - val) > abs(b[mid + 1] - val)) {
			low = mid + 1;
		} else {
			return b[mid];
		}
...
/* 
Sort one array and binary search over other
...
void solve() {
	...
	int val = INT_MAX;
	for (int i = 0; i < n; i++) {
		int curr_val = binsearch(b, a[i]);
		if (abs(curr_val - a[i]) < val) {
			p = {a[i], curr_val};
			val = abs(curr_val - a[i]);
		}
	}
