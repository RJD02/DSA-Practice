...
bool valid(string v) {
	if(v[0] == 'A' or v[0] == 'P' or v[0] == 'R' or v[0] == 'I' or v[0] == 'L')
		return true;
	return false;
}
...
void solve() {
	...
	// Find the count which matches the given criteria
	int count = 0;
	for(int i = 0; i < n; i++) {
		if(valid(v[i])) {
			count++;
	...
	int same = 0;
	for(int i = 1; i < sz(valid_string); i++) {
		if(valid_string[i][0] == valid_string[i - 1][0])
			same++;
	}
	if(same > 0)
		same++;
	if(count > 2)
		cout << count * (count - 1) * (count - 2) / (6 * factorial(same));
	else
		cout << 0;
	cout << endl;
}
