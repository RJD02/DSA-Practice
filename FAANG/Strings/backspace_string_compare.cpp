...
stack<char> builder(string s) {
	stack<char> st;
	for (int i = 0; i < sz(s); i++) {
		if (s[i] == '#' and not st.empty())
			st.pop();
		if (s[i] != '#')
			st.push(s[i]);
	}
...
void solve() {
	string s, t;
	cin >> s >> t;
	int index_s = sz(s) - 1, index_t = sz(t) - 1;
...
// 	string s, t;
// 	cin >> s >> t;
// 	stack<char> stack_s = builder(s);
// 	stack<char> stack_t = builder(t);
// 	if(sz(stack_t) != sz(stack_s)) {
// 		cout << "False";
// 		return;
// 	}
